...
<link rel="stylesheet" href="/ex/resources/css/mail_receivelist.css">
	...
	if (result=='add'){
    	...
    	$(document).ready(function(){
      		$('.search2').on("click",function(event){
      			...
      			self.location="/ex/mail/mail_receivelist"+'${pageMaker.makePage(1)}'
		...
		<div id="contents">
			...
			<div class="mail_bottom">
				<div class="mail_list">
					<form class="form" method="post">
						...
						<c:forEach var="list" items="${receivelist}">
							<div>
								<span class="c0"><input type="checkbox" name="mseq"
									value="${list.mseq}"></span>
								...
								<input type="hidden" name="mseq" value="${list.mseq}"
									class="mseq"> <input type="hidden" name="imp"
									value="${list.imp}" class="imp">

								<%-- <span class="c1"><div class="align1" >${list.eseq}</div></span> --%>
								<span class="c2" id="name">
									...
									href="mail_content?eseq=${userName}&mseq=${list.mseq}"><div class="align2">
											${list.title}</div></a></span> <span class="c4"><div class="align3">
				...
				<c:if test="${pageMaker.prev }">
					<a href="mail_receivelist${pageMaker.makeSearch(pageMaker.startPage-1)}&eseq=${userName}">&lt;</a>
				</c:if>
				...
				<c:forEach begin="${pageMaker.startPage }" end="${ pageMaker.endPage}" 
					...
					<c:choose>
						<c:when test="${idx eq pageMaker.page }">
							<a href='mail_receivelist${pageMaker.makeSearch(idx)}&eseq=${userName}'
							<c:out value="${pageMaker.page==idx?' class=active ':'' }"/>>
							 ${idx}</a>
						</c:when>
						<c:otherwise>
							<a href='mail_receivelist${pageMaker.makeSearch(idx)}&eseq=${userName}'> ${idx}</a>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				<c:if test="${pageMaker.next }">
					<a href="mail_receivelist${pageMaker.makeSearch(pageMaker.endPage+1)}&eseq=${userName}">&gt;</a>
				</c:if>
			</div>
				</div>
			</div>
