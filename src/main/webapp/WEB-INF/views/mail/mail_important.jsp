	...
	alert("주소록에 추가되었습니다.");
       ...
       $(document).ready(function(){
   		...
   		$(".delete_btn").on("click",function(){
   			fObjsct.attr("action","/ex/mail/impdelete");
   			fObjsct.submit();
   		});
   	});
   	...
   	$(document).ready(function(){
     		$('.search2').on("click",function(event){
     			...
     			self.location="/ex/mail/mail_important"+'${pageMaker.makePage(1)}'
                ...
                <div class="mail_page">
                    <div class="pagination">
                        <c:if test="${pageMaker.prev }">
							<a href="mail_important${pageMaker.makeSearch(pageMaker.startPage-1)}&eseq=${userName}">«</a>
						</c:if>
						...
						<c:forEach begin="${pageMaker.startPage }"
							...
							<c:choose>
								<c:when test="${idx eq pageMaker.page }">
									<a href='mail_important${pageMaker.makeSearch(idx)}&eseq=${userName}'>
										${idx}</a>
								</c:when>
								<c:otherwise>
									<a href='mail_important${pageMaker.makeSearch(idx)}&eseq=${userName}'> 
						...
						<c:if test="${pageMaker.next }">
							<a href="mail_important${pageMaker.makeSearch(pageMaker.endPage+1)}&eseq=${userName}">»</a>
						</c:if>
                    </div>
                </div>
            </div>
