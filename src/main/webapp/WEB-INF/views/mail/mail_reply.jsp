...
$(document).ready(function(){
	...
	$(".send_btn").on("click",function(){
		fObjsct.attr("action","/ex/mail/mail_reply?eseq=${userName}");
		fObjsct.submit();
	});
});
                ...
                <div class="write">
                	<form method="post">
                		<c:forEach var="list" items="${reply}" >
		                    ...
		                    <p>받는사람<input type="text"  id="receiver" name="receiver" style="border:1px solid #ccc; width:600px; height:30px; margin-bottom:5px;margin-left: 40px;" 
		                    value=" ${list.sender}"/>
		                    <button type="button"  class="address_btn" onclick="document.getElementById('addressModal').style.display='block'">주소록</button></p>
		                    <p>제목<input type="text" name="title"  style="border:1px solid #ccc; width:600px; height:30px; margin-bottom:5px;margin-left: 70px;" value="RE:"/></p>
		                    <p>파일첨부<input type="file" name="mFile" style="margin-left: 40px;"/></p>
		                    <textarea class="textarea" name="content" style="border:1px solid #ccc; width:1000px; height:500px; margin-top:5px;">&nbsp;
	        
	        
	        -----Original Message-----
	        From: ${list.sname} &lt; ${list.sender} &gt;
	        To: < ${list.rname } ${list.receiver} >;
	        Sent: <fmt:formatDate value="${list.indate}" pattern="yyyy-MM-dd HH:mm" /> 
	        Subject: ${list.title}
