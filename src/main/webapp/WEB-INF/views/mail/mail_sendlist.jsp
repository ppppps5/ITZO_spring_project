	...
	alert("주소록에 추가되었습니다.");
       ...
       $(document).ready(function(){
   		...
   		$(".delete_btn").on("click",function(){
   			fObjsct.attr("action","/ex/mail/senddelete");
   			fObjsct.submit();
   		});
   	});
...
$(document).ready(function(){
		$('.search2').on("click",function(event){
			...
			self.location="/ex/mail/mail_sendlist"+'${pageMaker.makePage(1)}'
        ...
        <div id="contents">
            ...
            <div class="mail_bottom">
				...
				<c:if test="${pageMaker.prev }">
					<a href="mail_sendlist${pageMaker.makeSearch(pageMaker.startPage-1)}&eseq=${userName}">&lt;</a>
				</c:if>
				...
				<c:forEach begin="${pageMaker.startPage }" end="${ pageMaker.endPage}" 
					...
					<c:choose>
						<c:when test="${idx eq pageMaker.page }">
							<a href='mail_sendlist${pageMaker.makeSearch(idx)}&eseq=${userName}'
							<c:out value="${pageMaker.page==idx?' class=active ':'' }"/>>
							 ${idx}</a>
						</c:when>
						<c:otherwise>
							<a href='mail_sendlist${pageMaker.makeSearch(idx)}&eseq=${userName}'> ${idx}</a>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				<c:if test="${pageMaker.next }">
					<a href="mail_sendlist${pageMaker.makeSearch(pageMaker.endPage+1)}&eseq=${userName}">&gt;</a>
				</c:if>
			</div>
                </div>
            </div>
