			...
			function() {
				$('.search2').on(
						...
						function(event) {
							...
							self.location = "/ex/approve/approve_decide"
		...
		<div id="contents">
			...
			<div class="approve_middle">
				<select name="searchType" class="search">
					...
					<option value="제목"
						<c:out value="${pageMaker.searchType eq '제목'?'selected':''}" ></c:out>>제목</option>
				</select> <input type="text" placeholder="미결결재검색" class="search" id="keyword"
					name="keyword">&nbsp; <input type="submit" class="search2"
					value="검색">
			...
			<div class="approve_bottom">
				<div class="approve_list">
					<c:forEach var="decide" items="${decide}">
						<div>
							<span class="c0"></span>
							<c:set var="star" value="${decide.myapprove}" />
							<c:choose>
							...
							<input type="hidden" name="aseq" value="${decide.aseq}"
								class="aseq"> <input type="hidden" name="myapprove"
								value="${decide.myapprove}" class="myapprove"> <span
								class="c1">
								<div class="align1">${decide.aseq}</div>
							</span> <span class="c2" id="name">
								<div class="dropdown">
									<div class="align1">${decide.writer}</div>
								</div>
							</span> <span class="c3"><a
								href="approve_content?aseq=${decide.aseq}&eseq=${userName}"><div
										class="align2">${decide.title}</div></a></span> <span class="c4"><div
									class="align3">
									<fmt:formatDate value="${decide.indate}" pattern="yyyy-MM-dd" />
								</div></span>
							<hr class="line2">
						</div>
					</c:forEach>
				...
				<div class="approve_page">
					...
					<div class="pagination">
						<c:if test="${pageMaker.prev }">
							<a
								href="approve_decide${pageMaker.makeSearch(pageMaker.startPage-1)}&eseq=${userName}">&lt;</a>
						</c:if>
						...
						<c:forEach begin="${pageMaker.startPage }"
							...
							<c:choose>
								<c:when test="${idx eq pageMaker.page }">
									<a
										href='approve_decide${pageMaker.makeSearch(idx)}&eseq=${userName}'
						...
						<c:if test="${pageMaker.next }">
							<a
								href="approve_decide${pageMaker.makeSearch(pageMaker.endPage+1)}&eseq=${userName}">&gt;</a>
						</c:if>
					</div>
				</div>
			</div>
