			...
			function() {
				$('.search2').on(
						...
						function(event) {
							...
							self.location = "/ex/approve/approve_each"
	...
	<div id="wrap">
		<div id="contents">
			<div class="approve_top">
				...
				<span class="smenu"> <a
					...
					<a href="/ex/approve/approve_decide?eseq=${userName}">미결함&nbsp;|&nbsp;
						<a href="/ex/approve/approve_each?eseq=${userName}">기결함</a>&nbsp;|&nbsp;
			...
			<div class="approve_middle">
				...
				<input type="text" placeholder="개인문서함검색" class="search" id="keyword" name="keyword">&nbsp; 
			...
			<div class="approve_bottom">
				<div class="approve_list">
					<c:forEach var="each" items="${each}">
						<div>
							<span class="c0"></span>
							<c:set var="star" value="${each.myapprove}" />
							<c:choose>
							...
							<span class="c1"><div class="align1">${each.aseq}</div></span> <input
								type="hidden" name="aseq" value="${each.aseq}" class="aseq">
							<input type="hidden" name="myapprove" value="${each.myapprove}"
								class="myapprove"> <span class="c2" id="name">
								<div class="dropdown">
									<div class="align1">${each.approval}</div>

								</div>
							</span> <span class="c3"><a
								href="approve_content?aseq=${each.aseq}&eseq=${userName}"><div
										class="align2">${each.title}</div></a></span> <span class="c4"><div
									class="align3">
									<fmt:formatDate value="${each.indate}" pattern="yyyy-MM-dd" />
								</div></span>
							<hr class="line2">
						</div>
					</c:forEach>
				...
				<div class="approve_page">
					...
					<div class="pagination">
						<c:if test="${pageMaker.prev }">
							<a
								href="approve_each${pageMaker.makeSearch(pageMaker.startPage-1)}&eseq=${userName}">&lt;</a>
						</c:if>
						...
						<c:forEach begin="${pageMaker.startPage }"
							...
							<c:choose>
								<c:when test="${idx eq pageMaker.page }">
									<a
										href='approve_each${pageMaker.makeSearch(idx)}&eseq=${userName}'
						...
						<c:if test="${pageMaker.next }">
							<a
								href="approve_each${pageMaker.makeSearch(pageMaker.endPage+1)}&eseq=${userName}">&gt;</a>
						</c:if>
					</div>
				</div>
			</div>
